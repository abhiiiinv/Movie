...
video = st.radio("Does this movie have a video?", options=[1,0], format_func=lambda x: binary_mapping[x])

budget = st.slider("Budget", min_value=10000, max_value=500000000, step=1000000)
popularity = st.slider("Popularity", min_value=0.0, max_value=600.0, step=0.1)
runtime = st.slider("Runtime (minutes)", min_value=60, max_value=300, step=1)
...
if st.button("Predict"):
    ...
    input_data = pd.DataFrame(
        [[adult, budget, popularity,runtime,original_language ,video, vote_average, vote_count]],
